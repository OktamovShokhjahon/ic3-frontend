...
export default function TestPage({ params }: { params: { level: string } }) {
  ...
  const startTest = async (
    ...
    try {
      ...
      // Special case: external iSpring test for Level 1, Full Practice (79 questions)
      if (params.level === "1" && type === "full") {
        // Served from frontend/public as /lvl-1-3/index.html
        setLegacyUrl("/lvl-1-3/index.html");
        setTestType(type);
        setTestStarted(true);
        setStartTime(Date.now());
        // 79 questions, give more time (120 minutes = 7200 seconds)
        setTimeLeft(7200);
        return;
      }
      ...
      // Special case: external iSpring test for Level 3, Questions 41-72
      if (params.level === "3" && type === "41-72") {
        // Served from frontend/public as /lvl-3-2/index.html (same as testpage10)
        setLegacyUrl("/lvl-3-2/index.html");
        setTestType(type);
        setTestStarted(true);
        setStartTime(Date.now());
        // 45-minute timer for additional questions
      ...
      // Special case: external iSpring test for Level 3, Questions 72-125
      if (params.level === "3" && type === "72-125") {
        // Served from frontend/public as /lvl-3-3/index.html (same as testpage11)
        setLegacyUrl("/lvl-3-3/index.html");
        setTestType(type);
        setTestStarted(true);
        setStartTime(Date.now());
        // 45-minute timer for additional questions
      ...
      // Special case: external iSpring test for Level 3, Questions 1-140 (new added)
      if (params.level === "3" && type === "1-140") {
        // Served from frontend/public as /lvl-3-5/index.html (same as testpage13)
        setLegacyUrl("/lvl-3-5/index.html");
        setTestType(type);
        setTestStarted(true);
        setStartTime(Date.now());
        // 90-minute timer for full practice
      ...
      const timeLimit = type === "1-140" ? 6300 : 2700;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {params.level === "3" ? (
              <>
                ...
                <button
                  key="41-72"
                  onClick={() => startTest("41-72")}
                  className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-6 rounded-xl hover:from-blue-600 hover:to-indigo-700 transition transform hover:scale-105"
                >
                  <h3 className="text-xl font-bold mb-2">Questions 41-72</h3>
                  <p className="text-sm opacity-90">Second part of Level 3</p>
                </button>
                <button
                  key="72-125"
                  onClick={() => startTest("72-125")}
                  className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-6 rounded-xl hover:from-blue-600 hover:to-indigo-700 transition transform hover:scale-105"
                >
                  <h3 className="text-xl font-bold mb-2">Questions 72-125</h3>
                  <p className="text-sm opacity-90">Third part of Level 3</p>
                </button>
                <button
                  key="full"
                  onClick={() => router.push("/testpage12")}
                  className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-6 rounded-xl hover:from-blue-600 hover:to-indigo-700 transition transform hover:scale-105"
                >
                  <h3 className="text-xl font-bold mb-2">Full Practice</h3>
                  <p className="text-sm opacity-90">All questions</p>
                </button>
                <button
                  key="1-140"
                  onClick={() => startTest("1-140")}
                  className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-6 rounded-xl hover:from-blue-600 hover:to-indigo-700 transition transform hover:scale-105"
                >
